            ...
            @foreach($meals as $one)
                <div class="grid_3">
                    <div class="card" style="width:200px">
                        <img class="card-img-top" src="{{asset('upload/'.$one->image)}}" alt="Card image" width="500" height="300">
                        <div class="card-body">
                            <h4 class="card-title">{{$one->name}}</h4>
                            <p class="card-text">Price: {{$one->price}} S.P</p>
                            <p class="card-text">Restaurant: {{$one->get_restaurant->name}}</p>
                            <p class="card-text">Ingredients: {{$one->ingredients}}</p>
                            @guest
                                @if (Route::has('user.login'))
                                    <a href="{{ route('user.login') }}" class="btn btn-primary">Order</a>
                                @endif
                            @else
                                <a href="/user/add/mealorder/{{$one->id}}" class="btn btn-primary">Order</a>
                            @endguest
                        </div>
                    </div>
                </div>
